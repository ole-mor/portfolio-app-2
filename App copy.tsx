...
function App() {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLImageElement>) => {
    ...
    // Define the maximum and minimum scale factors
    const maxScale = 1.5; // Maximum scale when cursor is at the center
    const minScale = 1;   // Minimum scale when cursor is at the edge
    ...
    // Clamp the scale factor between minScale and maxScale
    const clampedScale = Math.max(minScale, Math.min(maxScale, scale));
  
    // Apply the scale factor to the icon
